...
async def create_new_role(data: RoleCreate, db: AsyncSession = Depends(get_async_session)):
    return await create_role(db, data)

@router.put("/{role_id}", response_model=RoleOut)
async def edit_role( role_id: int, data: RoleCreate, db: AsyncSession = Depends(get_async_session)):
...
async def remove_role(role_id: int, db: AsyncSession = Depends(get_async_session)):
